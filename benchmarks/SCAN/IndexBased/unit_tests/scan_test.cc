...
TEST(ScanSubroutines, EmptyGraph) {
  ...
  ii::NeighborOrder neighbor_order{&graph, scan::CosineSimilarity{}};
  EXPECT_EQ(neighbor_order.size(), kNumVertices);

  auto seq = parlay::sequence<uintE>(4);
  std::cout << seq.empty() << std::endl;
  auto sl = make_slice(seq);
  std::cout << sl.empty() << std::endl;

//  std::cout << neighbor_order[0].empty() << std::endl;
//  EXPECT_THAT(neighbor_order[0], IsEmpty());
//  for (auto vertex_order : neighbor_order) {
