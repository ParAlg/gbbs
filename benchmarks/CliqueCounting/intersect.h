...
bool is_edge2(Graph& DG, uintE v, uintE u) {
  ...
  DG.get_vertex(u).out_neighbors().map(map_f2, false);
  return is;
}
...
struct HybridSpace_lw {
  ...
  void alloc(size_t max_induced, size_t k, size_t n, bool _use_old_labels,
    ...
    if (k > 1 && free_relabel && induced_degs == nullptr)
  ...
  void setup_nucleus(Graph& DG, Graph2& DG2, size_t k, uintE* base, size_t r,
    ...
    uintE nn0 = DG.get_vertex(g_vert_map(base[0])).out_degree() + 1;
    ...
    for (size_t j = 0; j <= r - 1; j++){
      ...
      DG.get_vertex(g_vert_map(base[k-j])).out_neighbors().map(map_label_f, false);
    }
    ...
    DG.get_vertex(g_vert_map(base[0])).out_neighbors().map(map_label_f, false); // r
    ...
    if (k-r == 1) {
      ...
      DG.get_vertex(g_vert_map(base[k])).out_neighbors().map(map_relabel_f, false);
      num_edges = 0;
      return;
    }
    ...
    auto map_f = [&] (const uintE& src, const uintE& v2, const W& wgh) {
      ...
      if (old_labels[v] > 0 && old_labels[v] < nn0) DG2.get_vertex(v).out_neighbors().map(map_nbhrs_f, false);
      j++;
    };
