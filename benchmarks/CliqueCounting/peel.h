...
sequence<bucket_t> Peel(Graph& G, Graph2& DG, size_t k, size_t* cliques, bool label, sequence<uintE> &rank,
  ...
  while (finished != n) {
    ...
    if (active.size() > 1) {
      ...
      if (k == 2) filter_size = triUpdate(G, DG, get_active, active.size(), granularity, still_active, rank, per_processor_counts, update_clique, true, update_changed);
      else filter_size = cliqueUpdate(G, DG, k, max_deg, label, get_active, active.size(), granularity, still_active, rank, per_processor_counts, update_clique, true, update_changed);
    }
    else {
...
// remove this_arr vertices ************************************************
    ...
    if (k == 2) triUpdate(G, DG, get_active, active_size, granularity, still_active, rank, per_processor_counts, update_clique, false, nop);
    else cliqueUpdate(G, DG, k, max_deg, label, get_active, active_size, granularity, still_active, rank, per_processor_counts, update_clique, false, nop);
