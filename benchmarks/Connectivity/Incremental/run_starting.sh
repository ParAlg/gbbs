...
batch_sizes=(10000000) #(100000 1000000 10000000 100000000 1000000000 10000000000)
  ...
  do
    ...
    do
      ...
      do
        ...
        do
          echo "sudo numactl -i all ./mains/${binary} -s -m -r ${rounds} -check -update_pct ${update_pct} -insert_to_query ${ins_to_query} -batch_size $batch_size $graph >> ${output_dir}/${NAME}_${update_pct}_${ins_to_query}_${batch_size}.data"
          sudo numactl -i all ./mains/${binary} -s -m -r ${rounds} -check -update_pct ${update_pct} -insert_to_query ${ins_to_query} -batch_size ${batch_size} $graph >> ${output_dir}/${NAME}_${update_pct}_${ins_to_query}_${batch_size}.data
        done
      done
    done
  done
