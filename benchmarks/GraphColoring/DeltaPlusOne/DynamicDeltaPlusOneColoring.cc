...
#include "DynamicDeltaPlusOneColoring.h"

namespace gbbs {
template <class Graph>
double DynamicColoring_runner(Graph& G, commandLine P) {
  std::cout << "### Application: Dynamic Coloring" << std::endl;
  ...
  // Options for palette size
  double palette_size_factor = P.getOptionDoubleValue("-psf", 1);
  ...
  // Run DynamicColoring
  timer t; t.start();
  RunDynamicColoring(G, batch_edge_list, batch_size, offset, palette_size_factor);
  double tt = t.stop();
...
generate_symmetric_main(gbbs::DynamicColoring_runner, false);
