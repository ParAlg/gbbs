...
std::tuple<ulong, double> Count5Cycle_runner(Graph& G, long order_type, bool experiment, bool escape, bool no_schedule, bool serial) {
...
double Count5Cycle_runner(Graph& G, commandLine P) {
  ...
  long esparsify_denom = P.getOptionLongValue("--edenom", 0); // probability of keeping an edge for edge sparsification
  ...
  if (sparsify) {
    ...
    if (sparsify_denom != 0) {
      ...
      auto ret = Count5Cycle_runner(G_sparse, order_type, experiment, escape, no_schedule, serial);
  ...
  return std::get<1>(Count5Cycle_runner(G, order_type, experiment, escape, no_schedule, serial));
}
}  // namespace gbbs
