...
struct sorted_it {
  ...
  size_t ct_b;
  size_t tot;
  sorted_it(uintE* ngh_a, uintE* ngh_b, uintE deg_a, uintE deg_b, uintE id_a, uintE id_b) : ngh_a(ngh_a), ngh_b(ngh_b), deg_a(deg_a), deg_b(deg_b), id_a(id_a), id_b(id_b) {
    ct_a = 0;
    ct_b = 0;
    tot = deg_a + deg_b;
  }
  ...
  std::pair<uintE, uintE> get_next() {
    if (ct_a < deg_a) {
      if (ct_b < deg_b) {
        ...
        return std::make_pair(std::min(ret.first, ret.second), std::max(ret.first, ret.second));
      }
    } else { /* exhausted a */
      assert(ct_b < deg_b);
