...
namespace connectit {
  ...
  sequence<edge> compose_algorithm_and_sampling(Graph& G, commandLine& P, Algorithm& alg) {
    if constexpr (sampling_option == sample_kout) {
      ...
      auto comp_alg = SamplingAlgorithmTemplate<Graph, KOut, Algorithm,
      ...
      auto sample = BFS(G, P);
      auto comp_alg = SamplingAlgorithmTemplate<Graph, BFS, Algorithm,
           algorithm_type, sampling_option>(G, sample, alg);
      return comp_alg.spanning_forest();
    }
      ...
      auto sample = LDD(G, P);
      auto comp_alg = SamplingAlgorithmTemplate<Graph, LDD, Algorithm,
           algorithm_type, sampling_option>(G, sample, alg);
      return comp_alg.spanning_forest();
    }
    else {
      static_assert(sampling_option == no_sampling);
      auto comp_alg = NoSamplingAlgorithmTemplate<Graph, Algorithm,
