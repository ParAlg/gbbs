...
#include "benchmarks/SpanningForest/UnionFind/Connectivity.h"
#include "benchmarks/SpanningForest/ShiloachVishkin/Connectivity.h"
#include "benchmarks/SpanningForest/LabelPropagation/Connectivity.h"
#include "benchmarks/SpanningForest/LiuTarjan/Connectivity.h"
...
namespace connectit {
  ...
  pbbs::sequence<edge> compose_algorithm_and_sampling(Graph& G, commandLine& P, Algorithm& alg) {
    if constexpr (sampling_option == sample_kout) {
      ...
      auto alg = SamplingAlgorithmTemplate<Graph, KOut, Algorithm,
           algorithm_type, sampling_option>(G, sample, alg);
      return alg.spanning_forest();
    }
    else if constexpr (sampling_option == sample_bfs) {
      using BFS = BFSSamplingTemplate<Graph>;
      auto sample = BFS(G, P);
      auto alg = SamplingAlgorithmTemplate<Graph, BFS, Algorithm,
           algorithm_type, sampling_option>(G, sample, alg);
      return alg.spanning_forest();
    }
      ...
      auto sample = LDD(G, P);
      auto alg = SamplingAlgorithmTemplate<Graph, LDD, Algorithm,
           algorithm_type, sampling_option>(G, sample, alg);
      return alg.spanning_forest();
    }
    else {
      static_assert(sampling_option == no_sampling);
      auto alg = NoSamplingAlgorithmTemplate<Graph, Algorithm,
           algorithm_type>(G, alg);
      return alg.spanning_forest();
    }
  }
