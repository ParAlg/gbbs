 ...
 public:
    ...
    if (use_block_v(u)) {
      ...
      if (lowD[u] > 0) {
        tmp = pack_neighbors_helper<StaticEdgeT, F>(
            tb1->find(u, NULL), u, seq_out.cut(offset, offset + D[u]));
      }
      if (lowD[u] < D[u]) {
        pack_neighbors_helper<StaticEdgeT, F>(
            tb2->find(u, NULL), u, seq_out.cut(offset + tmp, offset + D[u]));
      }
    }
  }
  ...
  DyGraph(int t_block_size, Graph& G, size_t t_n)
    ...
    par_for(0, n, [&](size_t i) {
      ...
      lowD[i] = G.get_vertex(i).out_neighbors().reduce(map_f, monoid);
      rbledLowD[i] = lowD[i];
    });
    ...
    parallel_for(0, n, [&](size_t i) {
      ...
      if (use_block(degree)) {
        ...
        G.get_vertex(i).out_neighbors().map_with_index(map_f2);
      } else {
        tableE* tb1 = LL;
        tableE* tb2 = LH;
        if (is_high_v(i)) {
