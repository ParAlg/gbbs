...
generateEdgeUpdates(size_t nv, size_t numEdges, const commandLine &P) {
...
//   std::cout << "[" << t_func << "] " << t_item << " = " << t_suffix <<
//   std::endl;
// }

template <class T> inline void PrintSubtimer(std::string t_item, T t_suffix) {
  std::cout << "::" << t_item << ": " << t_suffix << std::endl;
}

inline void PrintBreak() { std::cout << std::endl; }

template <typename A> inline void PrintVec(A *t_vec, int t_len) {
...
template <typename A> inline void PrintPairSeq(A t_vec) {
...
template <typename A> inline void PrintVec2(A *t_vec, int t_len) {
  for (int i = 0; i < t_len; ++i) {
    t_vec[i].print();
  }
  std::cout << std::endl;
...
template <class UT>
inline void staticCount(const std::vector<UT> &edges, int num_batch,
                        commandLine &P, size_t n) {
  //   size_t block_size = 0;
