...
#define generate_weighted_sage_main(APP)                                 \
  int main(int argc, char* argv[]) {                                     \
    ...
    if (compressed) {                                                    \
      if (symmetric) {                                                   \
        auto G = sage_io::read_compressed_symmetric_graph<int>(f1, f2);  \
        alloc_init(G);                                                   \
        run_app(G, APP, rounds)                                          \
      } else {                                                           \
        auto G = sage_io::read_compressed_asymmetric_graph<int>(f1, f2); \
        alloc_init(G);                                                   \
        run_app(G, APP, rounds)                                          \
      }                                                                  \
    } else {                                                             \
      if (symmetric) {                                                   \
        auto G = sage_io::read_symmetric_binary_graph<int>(f1, f2);      \
        alloc_init(G);                                                   \
        run_app(G, APP, rounds)                                          \
      } else {                                                           \
        auto G = sage_io::read_asymmetric_binary_graph<int>(f1, f2);     \
        alloc_init(G);                                                   \
        run_app(G, APP, rounds)                                          \
      }                                                                  \
    }                                                                    \
