...
namespace pbbs {
  ...
  size_t linear_search(Sequence const &I, typename Sequence::T v, const F& less) {
  ...
  template <typename Sequence, typename F> 
  size_t binary_search(Sequence const &I, typename Sequence::T v, const F& less) {
    size_t start = 0;
    size_t end = I.size();
  ...
  template <typename Sequence, typename F> 
  size_t linear_search(Sequence const &I, const F& less) {
    for (size_t i = 0; i < I.size(); i++)
  ...
  template <typename Sequence, typename F> 
  size_t binary_search(Sequence const &I, const F& less) {
    size_t start = 0;
    size_t end = I.size();
    while (end-start > _binary_search_base) {
      size_t mid = (end+start)/2;
