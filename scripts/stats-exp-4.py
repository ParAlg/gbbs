...
def main():
  ...
  program_pres = ["dblp", "brain", "wiki", "youtube", "stackoverflow",
  "livejournal", "orkut", "ctr", "usa"]
  pres = "plds"
  #empty = "empty_h"
  num_rounds = 3
  e = 0.4
  ...
  write_dir = "/home/qliu19/new-exp-4-delete-stats/"
  actual_batch_size = 10000
  #for b_idx, b in enumerate(batch_sizes):
  for p_idx, p in enumerate(program_pres):
    for b_idx, b in enumerate(batch_sizes):
        read_filename = write_dir + pres + "_" + str(p) + "_" + str(e) + "_" + str(d) + "_" + str(b) + "_60.out"
        ...
        print("{:.3f}".format(best_avg_error), end = " & ")
