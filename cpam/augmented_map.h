...
//  static M map_set(Ma a, const F& f) {return to_aug(Map::map_set(a, f));}
  ...
  static void foreach_cond_par(M m, F f, C c, size_t start=0,
			    size_t granularity = utils::node_limit) {
    Map::foreach_cond_par(m, f, c, start, granularity); }
public:
...
using diff_encoded_aug_map = aug_map<_Entry, Balance, diffencoded_entry_encoder>;
