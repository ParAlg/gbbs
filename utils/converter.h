...
namespace bytepd {
  ...
  void write_graph_bytepd_directed(Graph& GA, std::ofstream& out) {
    ...
    // 1. Calculate total size
    {
      ...
      parallel_for(0, n, [&] (size_t i) {
        ...
        if (deg > 0) {
          auto it = GA.get_vertex(i).out_neighbors().get_iter();
          size_t nbytes = bytepd::sequentialCompressEdgeSet<W>(edges.begin() + byte_offsets[i], 0, deg, (uintE)i, it, PAR_DEGREE_TWO);
          if (nbytes != (byte_offsets[i+1] - byte_offsets[i])) {
  ...
  template <class Graph>
  void write_graph_bytepd_format(Graph& GA, std::ofstream& out, bool symmetric, size_t n_batches = 6) {
    if (!symmetric) {
    ...
    for (size_t i=0; i<bs; i++) {
      ...
      parallel_for(start, end, [&] (size_t j) {
        ...
        if (deg > 0) {
          ...
          size_t nbytes = bytepd::sequentialCompressEdgeSet<W>(edges + our_offset, 0, deg, (uintE)j, it, PAR_DEGREE_TWO);
