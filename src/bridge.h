...
inline void free_array(void* a) { return pbbs::free_array(a); }
...
inline uint32_t hash32(uint32_t a) { return pbbs::hash32(a); }

// cheaper, likely more sketchy versions
inline uint32_t hash32_2(uint32_t a) { return pbbs::hash32_2(a); }

// cheaper, likely more sketchy versions
inline uint32_t hash32_3(uint32_t a) { return pbbs::hash32_3(a); }

// from numerical recipes
inline uint64_t hash64(uint64_t u) { return pbbs::hash64(u); }

// a slightly cheaper, but possibly not as good version
// based on splitmix64
inline uint64_t hash64_2(uint64_t x) { return pbbs::hash64_2(x); }
...
inline bool atomic_compare_and_swap(double* a, const double& oldval,
                                    const double& newval) {
...
inline auto scan(In_Seq const& In, Monoid m, flags fl = no_flag)
    -> std::pair<pbbs::sequence<typename In_Seq::value_type>,
...
void sample_sort_inplace(pbbs::range<Iter> A, const BinPred& f,
                         bool stable = false) {
...
void integer_sort_inplace(pbbs::range<T*> In, Get_Key const& g,
                          size_t key_bits = 0) {
  return pbbs::integer_sort_inplace(In, g, key_bits);
}

template <typename Seq, typename Get_Key>
pbbs::sequence<typename Seq::value_type> integer_sort(Seq const& In,
                                                      Get_Key const& g,
                                                      size_t key_bits = 0) {
...
sequence<size_t> count_sort(InS const& In, range<typename InS::value_type*> Out,
                            KeyS const& Keys, size_t num_buckets,
                            float parallelism = 1.0,
                            bool skip_if_in_one = false) {
  return pbbs::count_sort(In, Out, Keys, num_buckets, parallelism,
                          skip_if_in_one);
}
...
template <class intT>
pbbs::sequence<intT> random_permutation(size_t n,
                                        pbbs::random r = pbbs::random()) {
