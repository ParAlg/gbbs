...
struct sym_noop_manager {
  ...
  E* e0;
  E* e1;
  sym_noop_manager(vertex<W>& V0, vertex<W>& V1, const uintE vtx_id, const uintE block_size) :
    vtx_id(vtx_id),
...
struct compressed_sym_noop_manager {
  ...
  __attribute__((always_inline)) inline uintE block_degree(uintE block_num) {
    return bytepd_amortized::get_block_degree(e0, degree, block_num); // TODO: nvm
  }

  template <class F>
  inline void decode_block(uintE block_num, F f) {
    bytepd_amortized::template decode_block<W>(f, e0, vtx_id, degree, block_num);
  }

  template <class F>
  inline void decode_block_cond(uintE block_num, F f) {
    bytepd_amortized::template decode_block_cond<W>(f, e0, vtx_id, degree, block_num);
  }
};
