...
inline vertexSubsetData<data> edgeMapSparse(Graph& G,
  ...
  if (should_output(fl)) {
    ...
    size_t outEdgeCount = pbbslib::scan_add_inplace(make_slice(offsets));
...
inline vertexSubsetData<data> edgeMapBlocked(Graph& G, VS& indices, F& f,
  ...
  pbbslib::scan_add_inplace(make_slice(degrees), pbbslib::fl_scan_inclusive);
  ...
  cts[n_threads] = 0;
  size_t out_size = pbbslib::scan_add_inplace(make_slice(cts));
...
//  std::cout << "n_groups = " << n_groups << std::endl;
  ...
  size_t output_size = pbbslib::scan_add_inplace(make_slice(block_offsets));
