...
described in the Julienne paper [1], which we will discuss later. The
key point here is that instead of always returning the new bucket for
this vertex, we call the `B.get_bucket` primitive corresponding to the
new bucket for this vertex (`new_deg`). If the bucketing data
structure does not need to move this vertex from its current bucket to
the bucket corresponding to `new_deg` it returns a value of
`UINT_E_MAX`, which the `wrap` primitive then ignores by returning
`std::nullopt`. The other reason that a neighbor may not need to move
